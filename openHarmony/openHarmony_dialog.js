...
$.oDialog.prototype.confirm = function (
  labelText,
  title,
  okButtonText,
  cancelButtonText
  ...
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.confirm not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
...
$.oDialog.prototype.alert = function (labelText, title, okButtonText) {
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.alert not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
 ...
 */
$.oDialog.prototype.alertBox = function (
  labelText,
  title,
  okButtonText,
  htmlSupport
...
$.oDialog.prototype.toast = function (labelText, position, duration, color) {
  ...
  if (typeof position === "undefined") {
    ...
    var position = new $.oPoint(
      center.x(),
      center.y() + UiLoader.dpiScale(150)
    );
  }
 ...
 */
$.oDialog.prototype.browseForFile = function (
  text,
  filter,
  getExisting,
  acceptMultiple,
  ...
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.browseForFile not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
  ...
  if (getExisting) {
    if (acceptMultiple) {
      var _files = QFileDialog.getOpenFileNames(
        0,
        text,
        startDirectory,
        filter
...
$.oDialog.prototype.browseForFolder = function (text, startDirectory) {
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.browseForFolder not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
 ...
 */
$.oDialog.prototype.chooseFile = function (
  text,
  filter,
  getExisting,
  acceptMultiple,
  ...
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.chooseFile not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
  ...
  if (getExisting) {
    if (acceptMultiple) {
      var _chosen = QFileDialog.getOpenFileNames(
        0,
        text,
        startDirectory,
        filter
      ...
      var _chosen = QFileDialog.getOpenFileName(
        0,
        text,
        startDirectory,
        filter
  ...
  try {
    _chosen = _chosen.map(function (thisFile) {
      return new $.oFile(thisFile);
    });
  } catch (err) {
...
$.oDialog.prototype.chooseFolder = function (text, startDirectory) {
  if (this.$.batchMode) {
    this.$.debug(
      "$.oDialog.chooseFolder not supported in batch mode",
      this.$.DEBUG_LEVEL.WARNING
    );
    return;
  }
...
$.oProgressDialog = function (labelText, range, title, show) {
  ...
  if (!this.$.batchMode) {
    ...
    this.progress["canceled()"].connect(this, function () {
...
$.oProgressDialog.prototype.show = function () {
  if (this.$.batchMode) {
    this.$.debug(
      "$.oProgressDialog not supported in batch mode",
...
$.oPieMenu = function (
  ...
  if (typeof position === "undefined")
    var position = this.$.app.globalMousePosition;
...
$.oPieMenu.prototype.buildButton = function () {
  ...
  var icon =
    ...
    "/icons/brushpreset/defaultpresetellipse/ellipse03.svg";
...
$.oPieMenu.prototype.buildWidget = function () {
  ...
  for (var i = 0; i < this.widgets.length; i++) {
    ...
    var widgetPosition = new this.$.oPoint(
      center.x + itemPosition.x,
      center.y + itemPosition.y
    );

    widget.show();
    widget.move(
      widgetPosition.x - widget.width / 2,
      widgetPosition.y - widget.height / 2
    );
  }

  this.button.show();
  this.button.move(
    center.x - this.button.width / 2,
    center.y - this.button.height / 2
  );
};
...
$.oPieMenu.prototype.drawSlice = function () {
  ...
  MessageLog.trace(angleSlice);
  var slicePath = this.getSlicePath(
    center,
    angleSlice[0],
    angleSlice[1],
    this.minRadius,
  ...
  var contactPath = this.getSlicePath(
    center,
    this.minAngle,
    this.maxAngle,
    this.minRadius,
  ...
  sliceWidget.setStyleSheet("background-color: rgba(255, 0, 0, 0%);");
  if (this.$.app.version + this.$.app.minorVersion > 21) {
  ...
  sliceWidget.paintEvent = function () {
    try {
      ...
      // painter.setBrush(new QBrush(new QColor(backgroundColor)));
      // painter.drawPath(contactPath);
      painter.end();
      painter.restore();

      for (ix in pieMenuWidgets) {
        var bgColor = new $.oColorValue(pieMenuWidgets[ix].backgroundColor);
        painter.save();
        ...
        painter.translate(center.x, center.y);
        painter.rotate(sliceWidth * ix * -180);
        painter.translate(-center.x, -center.y);
        painter.setPen(new QPen(linesColor));
        painter.setBrush(
          new QBrush(new QColor(bgColor.r, bgColor.g, bgColor.b, 125))
        );
        painter.drawPath(slicePath);
        painter.end();
        painter.restore();
  ...
  sliceWidget.mouseMoveEvent = function (mousePos) {
    ...
    var angle =
      -position.add(-center.x, -center.y).polarCoordinates.angle / Math.PI;
    ...
    var indexWithinRange =
      currentIndex >= 0 && currentIndex < pieMenu.widgets.length;
    var distanceWithinRange =
      distance > pieMenu.minRadius && distance < pieMenu.maxRadius;
 ...
 * @private
 */
$.oPieMenu.prototype.getSlicePath = function (
  center,
  minAngle,
  maxAngle,
  minRadius,
  ...
  var smallArcBoundingBox = new QRectF(
    center.x - minRadius,
    center.y - minRadius,
    minRadius * 2,
    minRadius * 2
  ...
  smallArcStart.polarCoordinates = {
    radius: minRadius,
    angle: minAngle * -Math.PI,
  };
  ...
  var bigArcBoundingBox = new QRectF(
    center.x - maxRadius,
    center.y - maxRadius,
    maxRadius * 2,
    maxRadius * 2
...
$.oPieMenu.prototype.getItemAngleRange = function (index) {
  ...
  var angleStart =
    this.minAngle + (index / length) * (this.maxAngle - this.minAngle);
  var angleEnd =
    this.minAngle + ((index + 1) / length) * (this.maxAngle - this.minAngle);
...
$.oPieSubMenu.prototype.move = function (x, y) {
  ...
  this.button.move(
    localPos.x + this.widgetSize / 2 - this.button.width / 2,
    localPos.y + this.widgetSize / 2 - this.button.height / 2
  );
};
...
$.oPieSubMenu.prototype.buildWidget = function () {
  if (!this.parentMenu) {
    throw new Error(
      "must set parent first before calling $.oPieMenu.buildWidget()"
    );
  }
...
$.oPieButton = function (iconFile, text, parent) {
  ...
  var styleSheet =
    "QPushButton{ background-color: rgba(0, 0, 0, 1%); }" +
...
$.oToolButton = function (toolName, showName, iconFile, parent) {
  ...
  if (typeof iconFile === "undefined") {
    ...
    var scriptIconsFolder = new this.$.oFolder(
      specialFolders.resource + "/icons/drawingtool"
    );
    var iconFiles = scriptIconsFolder.getFiles(
      toolName.replace(" ", "").toLowerCase() + ".*"
    );
...
$.oActionButton = function (actionName, responder, text, iconFile, parent) {
  ...
  if (typeof iconFile === "undefined")
    var iconFile = specialFolders.resource + "/icons/old/exec.png";
...
$.oColorButton = function (paletteName, colorName, showName, parent) {
  ...
  iconMap.fill(
    new QColor(colorValue.r, colorValue.g, colorValue.b, colorValue.a)
  );
...
$.oScriptButton = function (scriptFile, scriptFunction, parent) {
  ...
  var scriptIconsFolder = new this.$.oFolder(
    scriptFile.folder.path + "/script-icons"
  );
...
$.oPrefButton = function (preferenceString, text, iconFile, parent) {
  ...
  if (typeof iconFile === "undefined")
    var iconFile =
      specialFolders.resource + "/icons/toolproperties/settings.svg";
  this.checkable = true;
